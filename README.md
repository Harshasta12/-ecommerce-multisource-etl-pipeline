...
Because the two sources are different businesses, there is no entity-level key (e.g. no shared customer_id). Integration is done at the category level instead see Integration Strategy below.
...
Storage: all layers live under /Volumes/dbacademy/default/ecommerce_project/{raw,bronze,silver,gold}/ Unity Catalog Volumes, not DBFS (see Design Decisions).
